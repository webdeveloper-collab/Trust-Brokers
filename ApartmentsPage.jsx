...
const ApartmentsPage = () => {
  ...
  const allApartments = getPropertiesByType('apartment')

  const filteredApartments = allApartments.filter(property => {
    const matchesLocation = searchLocation === '' || 
      property.location.toLowerCase().includes(searchLocation.toLowerCase())
    return matchesLocation
  })

  const totalPages = Math.ceil(filteredApartments.length / propertiesPerPage)
  const startIndex = (currentPage - 1) * propertiesPerPage
  const currentProperties = filteredApartments.slice(startIndex, startIndex + propertiesPerPage)
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-gray-500 max-w-2xl mx-auto">
              Explore our collection of modern apartments featuring contemporary design, premium amenities, and prime urban locations.
            </p>
          </div>
          ...
          <div className="mb-16">
            <div className="flex justify-center">
              <div className="relative max-w-md w-full">
                ...
                <input
                  type="text"
                  placeholder="Search apartments by location..."
          ...
          {searchLocation && (
            <div className="text-center mb-8">
              <p className="text-gray-600">
                {filteredApartments.length} {filteredApartments.length === 1 ? 'apartment' : 'apartments'} found in "{searchLocation}"
              </p>
            </div>
          )}
            ...
            {filteredApartments.length > 0 ? (
              ...
              <div className="col-span-full text-center py-16">
                ...
                <p className="text-gray-500 mb-6">
                  {searchLocation ? `No apartments found in "${searchLocation}"` : 'No apartments available at the moment.'}
                </p>
                <button
                  onClick={() => {
                    setSearchLocation('')
