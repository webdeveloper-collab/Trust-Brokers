...
const PlotsPage = () => {
  ...
  const allPlots = getPropertiesByType('plot')

  const filteredPlots = allPlots.filter(property => {
    const matchesLocation = searchLocation === '' || 
      property.location.toLowerCase().includes(searchLocation.toLowerCase())
    return matchesLocation
  })

  const totalPages = Math.ceil(filteredPlots.length / propertiesPerPage)
  const startIndex = (currentPage - 1) * propertiesPerPage
  const currentProperties = filteredPlots.slice(startIndex, startIndex + propertiesPerPage)
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-gray-500 max-w-2xl mx-auto">
              Discover prime residential and commercial plots in prestigious locations, perfect for building your dream property or investment.
            </p>
          </div>
          ...
          {searchLocation && (
            <div className="text-center mb-8">
              <p className="text-gray-600">
                {filteredPlots.length} {filteredPlots.length === 1 ? 'plot' : 'plots'} found in "{searchLocation}"
              </p>
            </div>
          )}
            ...
            {filteredPlots.length > 0 ? (
              ...
              <div className="col-span-full text-center py-16">
                ...
                <p className="text-gray-500 mb-6">
                  {searchLocation ? `No plots found in "${searchLocation}"` : 'No plots available at the moment.'}
                </p>
                <button
                  onClick={() => {
                    setSearchLocation('')
